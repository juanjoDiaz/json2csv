...
describe('Node Async Parser', () => {
  ...
  it('should not drain the whole input array before the consumer reads', async () => {
    ...
    const parser = new Parser(opts);
    // Intentionally not reading the output: a lazy source should stop
    // producing once its internal buffer/backpressure limit is reached,
    // instead of eagerly copying/draining the whole array upfront.
