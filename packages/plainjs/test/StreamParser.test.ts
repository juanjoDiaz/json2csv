...
import { StreamParser as Parser, type ParserOptions } from '@json2csv/plainjs';
...
describe('Stream Parser', () => {
  ...
  it('should support nested properties selectors', async () => {
    const opts: ParserOptions = {
      fields: [
        ...
        { label: 'Price', value: 'prices[0]' },
        { label: 'Color', value: 'color' },
        { label: 'Year', value: 'car.ye.ar' },
      ],
    };
  ...
  it('should process different combinations in fields option', async () => {
    const opts: ParserOptions = {
      fields: [
        ...
        { label: 'PATH1+PATH2', value: (row: any) => row.path1 + row.path2 },
        { label: 'NEST1', value: 'bird.nest1' },
        'bird.nest2',
        {
          label: 'nonexistent',
          value: 'fake.path',
  ...
  it("should use 'options.defaultValue' when no 'field.defaultValue'", async () => {
    const opts: ParserOptions<CarInfo> = {
      fields: [
        { value: 'carModel' },
        { label: 'price', value: (row: CarInfo) => row.price, default: 1 },
        { label: 'color', value: (row: CarInfo) => row.color },
      ],
      defaultValue: '',
    };
