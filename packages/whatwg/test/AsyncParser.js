...
import { number as numberFormatter, string as stringFormatter, stringExcel as stringExcelFormatter, stringQuoteOnlyIfNecessary as stringQuoteOnlyIfNecessaryFormatter } from '@json2csv/formatters';
import Parser from '../src/AsyncParser.js';

async function parseInput(parser, nodeStream) {
  return await parser.parse(nodeStream instanceof Readable ? Readable.toWeb(nodeStream) : nodeStream).promise();
}
