...
class JSON2CSVBase {
  ...
  preprocessOpts(opts) {
    ...
    processedOpts.unwindBlank = !Array.isArray(processedOpts.unwindBlank)
      ? (processedOpts.unwindBlank ? [processedOpts.unwindBlank] : [])
      : processedOpts.unwindBlank;
   ...
   */
  preprocessRow(row) {
    const processedRow = this.unwindData(row, this.opts.unwind, this.opts.unwindBlank);
  ...
  */
  unwindData(dataRow, unwindPaths, unwindBlankPaths) {
    const unwind = (rows, unwindPath, blank = false) => {
    ...
    const unwindBlank = (rows, path) => unwind(rows, path, true);

    const unwoundRows = unwindPaths.reduce(unwind, [dataRow]);
    return unwindBlankPaths.reduce(unwindBlank, unwoundRows);
  }
}
